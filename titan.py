...
SCAN_INTERVAL = 1800  # ۳۰ دقیقه - برای ماندن زیر سهمیه رایگان ۸۰۰ درخواست روزانه TwelveData
SCAN_LIST=['BTC/USD','EUR/USD','XAU/USD','ETH/USD','GBP/USD']  # فقط ۵ ارز پرطرفدار
WATCHLIST=set()
...
def fetch(sym, interval='15min', count=200):
    ...
    وقتی یه کلید به سهمیه روزانه‌اش رسید، خودکار میره سراغ کلید بعدی."""
    last_error = None
    attempts = len(TD_KEYS)
    for _ in range(attempts):
        ...
        try:
            ...
            if 'API credits' in msg or 'limit' in msg.lower() or d.get('code')==429:
                mark_key_exhausted()
                last_error = msg
                continue
            ...
            mark_key_exhausted()
            continue
    raise ValueError(f"همه کلیدهای API امروز به سهمیه‌شون رسیدن. {last_error or ''}")
...
async def callback_handler(update,context):
    ...
    if data=='analyze':
        ...
        await q.message.reply_text(
            ...
            f"⏱ هر چک: {SCAN_INTERVAL//60} دقیقه (برای ماندن زیر سهمیه API رایگان)\n\n"
