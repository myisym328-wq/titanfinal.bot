...
# ══════════════════════════════════════════════
#  چند کلید API برای TwelveData (Round-Robin)
#  هر کلید سهمیه روزانه جدا داره (مثلا 800 رایگان)
#  وقتی یکی تموم شد، خودکار میره سراغ بعدی
#  کلیدهایی که نداری رو خالی بگذار یا متغیرش رو ست نکن
# ══════════════════════════════════════════════
_td_keys_raw = [
    os.environ.get("TD_KEY", ""),
    os.environ.get("TD_KEY_2", ""),
    os.environ.get("TD_KEY_3", ""),
    os.environ.get("TD_KEY_4", ""),
    os.environ.get("TD_KEY_5", ""),
]
TD_KEYS = [k for k in _td_keys_raw if k and k != "YOUR_TWELVEDATA_API_KEY"]
if not TD_KEYS:
    TD_KEYS = ["YOUR_TWELVEDATA_API_KEY"]  # fallback تا حداقل کد کرش نکنه

_td_key_index = 0
_td_exhausted_today = set()  # ایندکس کلیدهایی که امروز سهمیه‌شون تموم شده
_td_exhausted_date = datetime.now().date()

def get_current_td_key():
    global _td_key_index, _td_exhausted_today, _td_exhausted_date
    # اگه روز عوض شده، سهمیه‌های exhausted رو ریست کن
    today = datetime.now().date()
    if today != _td_exhausted_date:
        _td_exhausted_today = set()
        _td_exhausted_date = today
    return TD_KEYS[_td_key_index % len(TD_KEYS)]

def mark_key_exhausted():
    """وقتی یه کلید به سهمیه‌اش رسید، اینو صدا بزن تا برو سراغ کلید بعدی"""
    global _td_key_index, _td_exhausted_today
    _td_exhausted_today.add(_td_key_index % len(TD_KEYS))
    _td_key_index += 1

def all_keys_exhausted():
    return len(_td_exhausted_today) >= len(TD_KEYS)
...
SCAN_INTERVAL = 600  # ۱۰ دقیقه - با ۵ کلید API سهمیه کافی برای دقت بیشتر در رصد TP/SL
SCAN_LIST=['BTC/USD','EUR/USD','XAU/USD','ETH/USD','GBP/USD']  # ۵ ارز اصلی
WATCHLIST=set()
scanner_active=False
scanner_tf='15min'
last_signals={}
pending_log={}  # log_id (int) -> trade data dict, waiting for user confirmation
pending_log_counter=0
...
def fetch(sym, interval='15min', count=200):
    """دریافت داده با چرخش خودکار بین چند کلید API.
    فقط وقتی خطای واقعی سهمیه (quota) باشه کلید رو 'تموم‌شده' علامت میزنیم.
    خطاهای موقت شبکه باعث سوختن کلید نمیشن، فقط یه‌بار با همون کلید دوباره تلاش میشه."""
    last_error = None
    keys_tried = set()
    max_total_attempts = len(TD_KEYS) * 2  # هر کلید حداکثر ۲ بار (یکی retry برای خطای موقت)

    for attempt in range(max_total_attempts):
        if all_keys_exhausted():
            break
        key = get_current_td_key()
        try:
            r=requests.get("https://api.twelvedata.com/time_series",
                params={"symbol":sym,"interval":interval,"outputsize":count,"apikey":key},timeout=15)
            d=r.json()
            if 'values' in d:
            ...
            code = d.get('code')
            # فقط این حالت‌ها یعنی واقعاً سهمیه تموم شده
            is_quota_error = (
                'API credits' in msg or
                'run out of' in msg.lower() or
                'daily limit' in msg.lower() or
                code == 429
            )
            if is_quota_error:
                mark_key_exhausted()
                last_error = msg
                continue
            # هر خطای دیگه (سیمبول نامعتبر، پارامتر اشتباه و...) واقعی و دائمیه، کلید رو نسوزون
            raise ValueError(msg or 'سیمبول پیدا نشد')

        except ValueError:
            raise
        except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
            # خطای شبکه موقت - کلید رو نسوزون، فقط یه‌بار دیگه با همون کلید امتحان کن
            last_error = f"خطای شبکه: {e}"
            if key in keys_tried:
                # این کلید رو قبلاً هم امتحان کردیم و بازم خطای شبکه داد، بریم بعدی بدون سوزوندن
    ...
    if all_keys_exhausted():
        raise ValueError(f"همه کلیدهای API امروز واقعاً به سهمیه‌شون رسیدن. {last_error or ''}")
    raise ValueError(f"دریافت داده ناموفق بود (مشکل شبکه احتمالی): {last_error or 'نامشخص'}")
...
async def do_analysis(msg_obj, symbol, tf, context=None):
    ...
    try:
        ...
        if sig['dir']!='WAIT' and context is not None:
            ...
            # رفع باگ: قیمت‌های اعشاری طولانی (مخصوصاً فارکس) از محدودیت ۶۴ بایتی
            # callback_data تلگرام رد میشن و دکمه بی‌صدا حذف میشه.
            # راه‌حل: دیتای معامله رو موقت توی حافظه نگه میداریم و فقط یه ID کوتاه میفرستیم.
            global pending_log_counter
            pending_log_counter = (pending_log_counter + 1) % 100000
            log_id = pending_log_counter
            pending_log[log_id] = {
                'symbol':symbol,'direction':sig['dir'],'entry':sig['entry'],
                'sl':sig['sl'],'tp1':sig['tp1'],'tp2':sig['tp2'],'tp3':sig['tp3'],
                'tp4':sig['tp4'],'tf':tf,'session':session_now
            }
            kb=InlineKeyboardMarkup([[InlineKeyboardButton(
                "📝 این معامله رو ثبت کن",
                callback_data=f"log_{log_id}"
            )]])
...
async def callback_handler(update,context):
    ...
    if data=='analyze':
        ...
        await q.message.reply_text(
            ...
            f"TF تحلیل: {scanner_tf} | ارزها: {len(SCAN_LIST)+len(WATCHLIST)}\n"
            f"⏱ هر چک: {SCAN_INTERVAL//60} دقیقه\n"
            f"🔑 تعداد کلید API فعال: {len(TD_KEYS)}\n\n"
            "سیگنال قوی → پیام فوری!\nهمچنین معاملات باز رو خودکار چک میکنه.\n\n"
            "⚠️ اگه ارز زیاد به واچ‌لیست اضافه کنی، ممکنه سهمیه API روزانه زودتر تموم بشه.",reply_markup=kb)
        ...
        try:
            ...
            trade_data=pending_log.get(log_id)
            if not trade_data:
                await q.message.reply_text("⚠️ این سیگنال منقضی شده (ربات ری‌استارت شده یا قدیمیه). دوباره تحلیل بگیر و سریع‌تر ثبتش کن.")
            else:
                tid=log_trade(trade_data['symbol'],trade_data['direction'],trade_data['entry'],
                               trade_data['sl'],trade_data['tp1'],trade_data['tp2'],
                               trade_data['tp3'],trade_data['tp4'],trade_data['tf'],"",
                               trade_data['session'])
                del pending_log[log_id]
                await q.message.reply_text(f"✅ معامله #{tid} ثبت شد!\nربات خودش رصد میکنه و وقتی به TP یا SL رسید بهت خبر میده.\n\n⚠️ یادت باشه اسکنر رو روشن نگه داری تا رصد انجام بشه.")
        except Exception as e:
            await q.message.reply_text(f"❌ خطا در ثبت: {e}")
    elif data=='top':
